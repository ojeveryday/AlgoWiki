...
#### 2. 从0开始的数组实现
从位置1开始直接好懂，不过会浪费0号位置这一个空间。我们仔细思考一下，发现从0位置开始也可以实现父子结点的对应关系。即，假设求父结点位置，我们先将index向左移动一位，即减一，然后再除以二。即可得到父节点位置。若求左孩子位置，则可以直接通过逆推求父结点的表达式得到，右孩子的位置则是左孩子位置加一。这里读者可以通过数学推导或者编写测试代码自行验证，帮助理解。
